...
public:
    ...
    {
        return glm::degrees(2.0f * atan(tan(glm::radians(mFov) * 0.5f) * mAspectRatio));
    }
    ...
    void getFrustum(float *left, float *top, float *right, float *bottom, float *near,
                    float *far) const;
